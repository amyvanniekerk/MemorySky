...
export default function GalaxyView({ memories, constellations, onStarPress, onHiddenStarPress, onStarDragStart, onStarDragMove }: GalaxyViewProps) {
  ...
  const constellationLines = useMemo(() => {
    const starMap = new Map(stars.map((s) => [s.memory.id, s]));
    const lines: { x1: number; y1: number; x2: number; y2: number; color: string }[] = [];

    if (constellations.length > 0) {
      // User-created constellations
      constellations.forEach((c) => {
        const validStars = c.memoryIds
          .map((id) => starMap.get(id))
          .filter((s): s is StarPosition => !!s);
      ...
      // Fallback: auto-generated by category
